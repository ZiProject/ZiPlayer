...
export class QueueController {
	...
	public setWillNext(track: Track | null): void { if (track) this.queue.willNextTrack(track); this.publishChanged(); }
	public clearWillNext(): void { this.queue.clearWillNext(); this.publishChanged(); }
	public get relatedTracks(): Track[] | null { return this.queue.relatedTracks(); }
	public setRelated(tracks: Track[]): void { this.queue.relatedTracks(tracks); this.publishChanged(); }
	public dispose(): void { this.detachAction?.(); for (const detach of this.detachQueries.splice(0)) detach(); this.queue.reset(); }
