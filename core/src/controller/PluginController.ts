...
export class PluginController {
	...
	constructor(bus: Bus) {
		...
		bus.registerRpc<{ name: string }, BasePlugin | undefined>("plugin.get", ({ name }, ctx) =>
			this.manager(ctx.playerId)?.get(name),
		);
		bus.registerRpc<{ plugin: BasePlugin }, void>("plugin.add", ({ plugin }, ctx) =>
		...
		bus.registerRpc<void, object>("plugin.stats", (_req, ctx) => {
			const manager = this.manager(ctx.playerId);
			if (!manager) throw new Error("PluginController has no manager for this player");
			return manager.getStats();
		});
		bus.registerRpc<{ track: Track; history?: Track[] }, Track[]>("plugin.relatedTracks", async ({ track, history }, ctx) => {
