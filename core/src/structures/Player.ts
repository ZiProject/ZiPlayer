...
import { PlayerBus, createPlayerRequestId, type PlayerInput, type PlayerAction as PlayerActionMessage, type PlayerEvent, type PlayerEventType, type PlayerQuery, type PlayerQueryMap } from "./PlayerBus";
...
export class Player extends EventEmitter {
	...
	public constructor(guildId: string, options: PlayerOptions = {}, manager: PlayerManager) {
		super();
		this.guildId = guildId; this.manager = manager;
		...
		this.connectionController = graph.connectionController; this.lifecycleController = graph.lifecycleController; this.forwardController = graph.forwardController; this.queue = graph.queue; this.audioPlayer = graph.audioPlayer; this.streamManager = graph.streamManager; this.preloadManager = graph.preloadManager; this.pluginManager = graph.pluginManager; this.extensionManager = graph.extensionManager; this.queueController = graph.queueController; this.trackLoader = graph.trackLoader; this.playbackController = graph.playbackController; this.streamController = graph.streamController; this.saveController = graph.saveController; this.filterController = graph.filterController; this.antiStuckController = graph.antiStuckController; this.transitionController = graph.transitionController; this.volumeController = graph.volumeController; this.preloadController = graph.preloadController; this.orchestrator = graph.orchestrator; this.ttsController = graph.ttsController; this.debugTracer = graph.debugTracer; this.searchController = graph.searchController; this.eventBridge = graph.eventBridge; this.filter = this.filterController;
		this.detachConnectionSubscription = this.bus.onOutput("[Connection]->[Player]:connected", (event) => { if (this.connection === event.connection) return; this.audioPlayerSubscription?.unsubscribe(); this.audioPlayerSubscription = event.connection.subscribe(this.audioPlayer) ?? null; this.ttsController.setConnection(event.connection); this.connection = event.connection; debug(`[Player] AudioPlayer subscribed guild=${guildId} session=${event.sessionId}`); });
		this.bus.onOutput("[Connection]->[Player]:disconnected", (event) => { this.audioPlayerSubscription?.unsubscribe(); this.audioPlayerSubscription = null; this.ttsController.setConnection(null); this.connection = null; debug(`[Player] AudioPlayer unsubscribed guild=${guildId} reason=${event.reason ?? "unknown"}`); });
		...
		if (Array.isArray(this.options.filters) && this.options.filters.length > 0) void this.filterController.applyFilters(this.options.filters).catch((error) => debug("[FilterController] Initial filter error:", error));
		this.bus.publish("initialized"); this.bus.publish("ready");
	}
	...
	public get isIdle(): boolean { if (this.playbackMode === "FORWARD") return this.forwardLeader?.isIdle ?? true; return this.bus.querySync("playerState") === "idle"; }
	public get isBuffering(): boolean { if (this.playbackMode === "FORWARD") return this.forwardLeader?.isBuffering ?? false; return this.bus.querySync("playerState") === "buffering"; }
	public get volume(): number { return this.bus.querySync("volume"); }
	public set volume(value: number) { this.volumeControllerSetVolumeViaBus(value); }
	...
	public get availablePlugins(): BasePlugin[] { return this.bus.querySync("availablePlugins"); }
	public get relatedTracks(): Track[] | null { return this.bus.querySync("relatedTracks"); }
	...
	public destroyCurrentStream(): void { this.streamController.abortCurrent(); this.playbackController.stop(); }
	public generateWillNext(): Track | null { return this.queueController.willNext; }
	public preloadNextTrack(): Promise<void> { return this.trackLoader.preloadNext(); }
	public safeCancelPreload(): Promise<void> { return this.preloadController.cancelSafely(); }
	...
	public getTrackTargetVolume(track: Track): number { const settings = this.volumeController.settings; const lufs = Number(track?.metadata?.lufs); if (!settings.enabled || !Number.isFinite(lufs)) return this.volume / 100; const correctionDb = Math.max(-settings.maxCutDb, Math.min(settings.maxBoostDb, settings.targetLUFS - lufs)); const gain = Math.pow(10, correctionDb / 20); const ceiling = Math.pow(10, settings.limiterCeiling / 20); return Math.min((this.volume / 100) * gain, ceiling); }
	public attemptTrackRecovery(track: Track, session?: PlaybackSession): Promise<TrackLoadResult> { if (!session) return Promise.reject(new Error("attemptTrackRecovery requires an active PlaybackSession")); return this.trackLoader.loadWithRecovery(track, session); }
	public promotePreloadToCurrent(track: Track): AudioResource | null { const session = this.orchestrator.currentSession; if (!session) return null; return this.preloadController.promote(track, { resource: null, track: null, streamId: null, processedStreamId: null, abortController: null, isValid: false, isLoading: false, loadPromise: null } as any); }
	public createResource(stream: Stream.Readable, track: Track): AudioResource { return this.playbackController.createResource(stream, track); }
	public mergeTrackPreserveRef(target: Track, source: Track): Track { Object.assign(target, source); return target; }
	public async applyTrackMiddleware(track: Track): Promise<Track> { const middleware: PlayerOptions["trackMiddleware"] = this.options.trackMiddleware; if (!Array.isArray(middleware)) return track; for (const fn of middleware) { const result = await fn(track, { player: this, manager: this.manager }); if (result && result !== track) Object.assign(track, result); } return track; }
	public async getStream(track: Track): Promise<StreamInfo | TrackLoadResult | null> { const session = this.orchestrator.currentSession; if (session) return this.trackLoader.load(track, session); const extensionStream = await this.extensionManager?.provideStream?.(track); return extensionStream ?? this.pluginManager?.getStream?.(track); }
	...
	public loadFreshStream(track: Track, session: PlaybackSession): Promise<TrackLoadResult> { return this.trackLoader.load(track, session ?? this.orchestrator.currentSession); }
	...
	public ensureTTSPlayer(): boolean { return !!this.ttsController?.ttsPlayer; }
	public interruptWithTTSTrack(track: Track, ..._args: any[]): Promise<boolean> { return this.play(track); }
	public previous(): Track | null { return this.queueController.previous(); }
	async save(track: Track, options?: SaveOptions | string): Promise<Stream.Readable> { try { return await this.saveController.save(track, options); } catch (error) { this.debug("[Player] save error:", error); this.emit("playerError", error as Error, track); throw error; } }
	async saveVideo(track: Track, options?: SaveVideoOptions | string): Promise<Stream.Readable> { if (!track) throw new TypeError("A track is required to save video"); try { return await this.saveController.saveVideo(track, options); } catch (error) { this.debug("[Player] saveVideo error:", error); this.emit("playerError", error as Error, track); throw error; } }
	public loop(mode?: any): any { return mode === undefined ? this.queueController.loop : this.queueController.setLoop(mode); }
	public autoPlay(enabled?: boolean): boolean { return enabled === undefined ? this.queueController.autoPlay : this.queueController.setAutoPlay(enabled); }
	public setVolume(value: number): number { return this.volumeController.setVolume(value); }
	public shuffle(): void { this.queue.shuffle(); this.bus.publish("queueChanged", this.queueController.snapshot()); }
	public clearQueue(): void { this.queueController.clear(); }
	public async insert(query: string | Track | Track[], index = 0, requestedBy?: string): Promise<boolean> { try { const tracks = typeof query === "string" ? (await this.search(query, requestedBy || "Unknown")).tracks : Array.isArray(query) ? query : [query]; if (tracks.length === 0) return false; for (let i = 0; i < tracks.length; i++) this.queueController.insert(tracks[i], index + i); return true; } catch (error) { this.debug("[Player] Insert error:", error); this.emit("playerError", error as Error); return false; } }
	public remove(index: number): Track | null { return this.queueController.remove(index); }
	public scheduleLeave(): void { this.lifecycleController.scheduleLeave?.(); }
	public clearLeaveTimeout(): void { this.lifecycleController.clearLeaveTimeout?.(); }
	...
	public getExtensions(): any[] { return this.extensionManager?.getAll?.() ?? []; }
	public setupEventListeners(): void {}
	public saveSession(_options?: any): any { return this.getSerializableState(); }
	public exitRemoteMode(): void { this.playbackMode = "NATIVE"; }
	public getSerializableState(): any { return { guildId: this.guildId, queue: this.queue.toJSON?.(), volume: this.volume, playbackMode: this.playbackMode }; }
