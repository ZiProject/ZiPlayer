...
export class Player extends EventEmitter {
 ...
 private bindLegacyCompatibility():void{
  ...
  for(const key of Object.keys(legacy)){
   ...
   try{(this as any)[key]=legacy[key];}catch{}
  }
 ...
 public debug(message?:any,...optionalParams:any[]):void{(this.legacy as any).debug?.(message,...optionalParams);}

 public search(query:string,requestedBy:string):Promise<SearchResult>{return this.searchController.search(query,requestedBy);} public clearSearchCache():void{this.searchController.clear();} public debugSearchQuery(query:string):ReturnType<SearchController["debug"]>{return this.searchController.debug(query);}
 public connect(channel:VoiceChannel):Promise<VoiceConnection>{return this.bus.request({type:"[Player]->[Connection]:connect",requestId:createPlayerRequestId(),channel}).then(e=>e.connection);} public disconnect():Promise<void>{return this.bus.request({type:"[Player]->[Connection]:disconnect",requestId:createPlayerRequestId()}).then(()=>undefined);}
 public play(query:string|Track|SearchResult|null,requestedBy?:string):Promise<boolean>{if(query===null)return this.action({type:"SKIP"}).then(()=>true);const track="tracks" in (query as any)?(query as SearchResult).tracks[0]:typeof query==="string"?undefined:query as Track;if(track)return this.action({type:"PLAY",track}).then(()=>true);return this.search(query,requestedBy||"Unknown").then(r=>r.tracks.length?this.action({type:"PLAY",track:r.tracks[0]}).then(()=>true):false).catch(()=>false);}
 public playNext():Promise<boolean>{return this.action({type:"SKIP"}).then(()=>true).catch(()=>false);} public pause():boolean{void this.action({type:"PAUSE"});return true;} public resume():boolean{void this.action({type:"RESUME"});return true;} public stop():boolean{void this.action({type:"STOP"});return true;}
 public seek(position:number):Promise<boolean>{return this.action({type:"SEEK",position}).then(()=>true).catch(()=>false);} public skip(index?:number):boolean{void this.action({type:"SKIP"});return true;}
 public subscribeTo(leader:Player,options?:{forwardMode?:boolean}):boolean{return this.forwardController.subscribeTo(leader,options);} public unsubscribeForward(reason?:string):boolean{return this.forwardController.unsubscribeForward(reason);}
 ...
 public override destroy():void{if(this.destroyed)return;this.destroyed=true;this.runtime.dispose();this.legacy.destroy();this.removeAllListeners();}
}
