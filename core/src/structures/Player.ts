...
export class Player extends EventEmitter {
	...
	public connect(channel: VoiceChannel): Promise<VoiceConnection> { return this.bus.request({ type: "[Player]->[Connection]:connect", requestId: createPlayerRequestId(), channel }).then((e) => e.connection); }
	public disconnect(): Promise<void> { return this.bus.request({ type: "[Player]->[Connection]:disconnect", requestId: createPlayerRequestId() }).then(() => undefined); }
	public play(query: string | Track | SearchResult | null, requestedBy?: string): Promise<boolean> {
		...
		if (typeof query === "string") return this.search(query, requestedBy || "Unknown").then((r) => r.tracks.length ? this.action({ type: "PLAY", track: r.tracks[0] }).then(() => true) : false).catch(() => false);
		const track = "tracks" in query ? query.tracks[0] : query;
		return track ? this.action({ type: "PLAY", track }).then(() => true) : Promise.resolve(false);
	}
	public playNext(): Promise<boolean> { return this.action({ type: "SKIP" }).then(() => true).catch(() => false); }
