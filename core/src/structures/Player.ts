...
export class Player extends EventEmitter {
	...
	public get lifecycleController() { return this.runtime.lifecycleController; }
	...
	public get ttsController() { return this.runtime.ttsController; }
	...
	public async play(query: string | Track | SearchResult | null, requestedBy?: string): Promise<boolean> {
		...
		if (this.isPlaying || this.isPaused) { void this.preloadController.preload().catch((error: unknown) => this.debug("[Player] Preload after queue add error:", error)); return true; }
		return this.playNext();
	}
	public async playNext(): Promise<boolean> { if (this.destroyed) return Promise.resolve(false); return this.action({ type: "SKIP" }).then(() => this.isPlaying || this.currentTrack !== null).catch(() => false); }
	...
	public insert(track: Track, index = 0): number { return this.queueController.insert(track, index); }
	...
	public destroy(): void { if (this.destroyed) return; this.destroyed = true; this.eventBridge.dispose(); this.runtime.dispose(); this.emit("playerDestroy"); this.removeAllListeners(); }
}