...
export class PlayerBusRequestError extends Error {
	public constructor(public readonly reason: PlayerBusRequestErrorReason, public readonly inputType: string, message: string) { super(message); this.name = "PlayerBusRequestError"; }
}

export type PlayerQuery = keyof PlayerQueryMap;
export interface PlayerQueryMap { currentTrack: Track | null; playerState: PlaybackSessionSnapshot["status"] | "idle"; queue: Track[]; playbackSession: PlaybackSessionSnapshot | null; position: number | null; volume: number; isPlaying: boolean; isPaused: boolean; }
	...
	public request<K extends PlayerRequestInputType>(input: Extract<PlayerInput, { type: K }>, options: PlayerRequestOptions<K> = {}): Promise<Reply<K>["success"]> {
		...
		return new Promise((resolve, reject) => {
			...
			if (options.signal) {
				...
				options.signal.addEventListener("abort", abort, { once: true }); cleanups.push(() => options.signal?.removeEventListener("abort", abort));
			}
