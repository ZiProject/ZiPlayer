...
export interface PlayerRequestOptions<K extends PlayerRequestInputType = PlayerRequestInputType> {
	timeoutMs?: number;
	signal?: AbortSignal;
	onProgress?: (event: Progress<K>) => void;
}
		...
		return new Promise((resolve, reject) => {
			...
			cleanups.push(
				this.onOutput(contract.success, (event) => {
					if (event.requestId === requestId) settle(() => resolve(event as Reply<K>["success"]));
				}),
			);
			cleanups.push(
			...
			if (contract.progress && options.onProgress)
				cleanups.push(
					this.onOutput(contract.progress, (event) => {
						if (event.requestId === requestId && !settled) options.onProgress!(event as Progress<K>);
					}),
				);
