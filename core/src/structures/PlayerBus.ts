...
const REQUESTS: Record<PlayerRequestInputType, RequestContract> = {
	...
	"[Player]->[Connection]:disconnect": {
		success: "[Connection]->[Player]:disconnected",
		error: "[Connection]->[Player]:error",
	},
	...
	"[Player]->[Resource]:refresh": {
		success: "[Resource]->[Player]:refreshed",
		error: "[Resource]->[Player]:error",
	},
};
...
export const createPlayerSessionId = (): string => `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
