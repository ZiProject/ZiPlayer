...
export class PreloadManager {
	...
	private readonly preloadSlot: StreamSlot = {
		resource: null, track: null, streamId: null, processedStreamId: null,
		abortController: null, isValid: false, isLoading: false, loadPromise: null,
	};
	...
	/** Atomically transfers the preloaded stream out of the preload slot. */
	public takePreloaded(track: Track): PromotedPreload | null {
		...
		if (streamId) {
			this.streamManager.updateMetadata(streamId, { isPreload: false, priority: 10 });
		}
	...
	public promoteToCurrent(track: Track, currentSlot: StreamSlot): AudioResource | null {
		...
		currentSlot.resource = createAudioResource(promoted.stream as any, { inlineVolume: true, metadata: { ...track, preloaded: true } });
	...
	public async preloadNextTrack(): Promise<void> {
		...
		if (!this.isEnabled()) { this.debugLog(`[Preload] Disabled by options/runtime profile`); return; }
		if (this.preloadLock) { this.debugLog(`[Preload] Already preloading, skipping`); return; }
		const nextTrack = this.getNextTrack();
		if (!nextTrack) { this.debugLog(`[Preload] No next track to preload`); return; }
		if (this.hasValidPreload(nextTrack)) { this.debugLog(`[Preload] Already have valid preload for: ${nextTrack.title}`); return; }
		...
		try {
			...
			if (err instanceof Error && err.message === "PRELOAD_CANCELLED") this.debugLog(`[Preload] Cancelled for ${nextTrack.title}`);
			else if (err instanceof Error && err.message === "No stream available") { this.debugLog(`[Preload] Skipped unplayable track: ${nextTrack.title}`); this.clearPreloadSlot(); this.preloadNext = true; }
			else { this.debugLog(`[Preload] Failed for ${nextTrack.title}:`, err); this.clearPreloadSlot(); }
		} finally { this.preloadLock = false; this.preloadSlot.isLoading = false; this.preloadSlot.loadPromise = null; }
	...
	public async safeCancelPreload(): Promise<void> {
		...
		if (this.preloadSlot.resource) {
			try { const stream = this.preloadSlot.resource.playStream; if (stream && typeof stream.destroy === "function" && !stream.destroyed) stream.destroy(); } catch {}
		}
	...
	public clearPreloadSlot(): void {
		...
		if (this.preloadSlot.streamId) this.streamManager.unregisterStream(this.preloadSlot.streamId, true);
		this.preloadSlot.resource = null; this.preloadSlot.track = null; this.preloadSlot.streamId = null;
		this.preloadSlot.abortController = null; this.preloadSlot.isValid = false; this.preloadSlot.isLoading = false; this.preloadSlot.loadPromise = null;
	}
	...
	private async executePreload(track: Track, abortController: AbortController): Promise<void> {
		...
		if (abortController.signal.aborted || this.isDestroyed()) { this.destroyStreamInfo(streamInfo); throw new Error("PRELOAD_CANCELLED"); }
		if (!this.trackMatches(this.getNextTrack(), track)) { this.destroyStreamInfo(streamInfo); throw new Error("PRELOAD_CANCELLED"); }
		if (!streamInfo?.stream && !streamInfo?.url) { if (this.removeTrackFromQueue?.(track)) this.debugLog(`[Preload] Removed unplayable track from queue: ${track.title}`); throw new Error("No stream available"); }
		const streamId = streamInfo.stream ? this.streamManager.registerStream(streamInfo.stream, track, { source: track.source || "preload", isPreload: true, priority: 5 }) : null;
		this.preloadSlot.streamId = streamId;
		try {
			const resource = createAudioResource(streamInfo.stream || streamInfo.url!, { inlineVolume: true, metadata: { ...track, preloaded: true } });
			if (abortController.signal.aborted || this.isDestroyed()) { try { resource.playStream?.destroy?.(); } catch {} throw new Error("PRELOAD_CANCELLED"); }
			if (!resource.playStream || resource.playStream.readable === false) throw new Error("Resource not readable");
			this.preloadSlot.resource = resource; this.preloadSlot.isValid = true; this.preloadSlot.track = track;
			this.debugLog(`[Preload] Successfully preloaded: ${track.title} (Stream ID: ${streamId})`);
		} catch (error) { if (streamId) this.streamManager.unregisterStream(streamId, true); this.preloadSlot.streamId = null; throw error; }
	}

	private destroyStreamInfo(streamInfo: StreamInfo | null): void {
		const stream = streamInfo?.stream;
		if (!stream) return;
		try { if (typeof stream.destroy === "function" && !stream.destroyed) stream.destroy(); } catch (error) { this.debugLog(`[Preload] Error destroying abandoned stream:`, error); }
	}

	private async getStreamWithCancel(track: Track, signal: AbortSignal): Promise<StreamInfo | null> {
		if (this.isDestroyed()) throw new Error("PLAYER_DESTROYED");
		let abortHandler: (() => void) | null = null; let settled = false;
		const abortPromise = new Promise<never>((_, reject) => { if (signal.aborted) { reject(new Error("PRELOAD_CANCELLED")); return; } abortHandler = () => reject(new Error("PRELOAD_CANCELLED")); signal.addEventListener("abort", abortHandler, { once: true }); });
		const existingStream = this.streamManager.getStreamByTrack(track.id || track.title);
		if (existingStream && !existingStream.destroyed && existingStream.readable !== false) { if (abortHandler) signal.removeEventListener("abort", abortHandler); return { stream: existingStream, type: "arbitrary" }; }
		const streamPromise = this.getStream(track);
		void streamPromise.then(result => { if (signal.aborted || this.isDestroyed()) this.destroyStreamInfo(result); }, () => undefined);
		try { const result = await Promise.race([streamPromise, abortPromise]); settled = true; return result as StreamInfo | null; }
		finally { if (!settled && signal.aborted) {} if (abortHandler) signal.removeEventListener("abort", abortHandler); }
	}
}
