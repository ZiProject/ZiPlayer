...
export class PlaybackOrchestrator {
	...
	private async handleAction(action: PlayerAction): Promise<void> {
		switch (action.type) {
			...
			case "PAUSE": this.playbackController?.pause(); this.session?.markPaused(); this.publishState(); return;
			case "RESUME": this.playbackController?.resume(); this.session?.markPlaying(); this.publishState(); return;
			case "SEEK": this.session?.updatePosition(action.position); this.publishState(); return;
			case "STOP": this.playbackController?.stop(); this.antiStuckController?.clear(this.session ?? undefined); this.session?.markStopped(); this.streamController?.abortCurrent(); this.trackLoader?.cancelPreload(); this.publishState(); return;
			case "SKIP": this.playbackController?.stop(); if (this.session) { this.antiStuckController?.clear(this.session); this.session.markEnded(); this.bus.event({ type: "TRACK_END", session: this.session.snapshot() }); } this.streamController?.abortCurrent(); this.trackLoader?.cancelPreload(); return;
	...
	private async start(track: Track): Promise<void> {
		...
		if (!this.trackLoader || !this.streamController || !this.playbackController) { this.bus.publish("playbackSessionCreated", session.snapshot()); this.bus.publish("trackRequested", track, session); return; }
		try {
