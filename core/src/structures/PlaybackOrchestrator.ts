...
export class PlaybackOrchestrator {
	...
	private async refreshResource(position: number, rpcContext: PlayerBusRpcContext): Promise<PlaybackSessionSnapshot> {
		...
		try {
			...
			if (!isCurrentRefresh()) throw new Error("Playback resource refresh superseded");
			const resource = this.bus.requestRpcSync<{ stream: import("stream").Readable; track: Track; inputType?: import("@discordjs/voice").StreamType }, import("@discordjs/voice").AudioResource>("resource.create", { stream: active.stream, track: session.track, inputType: active.inputType });
			if (!isCurrentRefresh()) throw new Error("Playback resource refresh superseded");
			session.setResource(resource);
	...
	dispose() {
		...
		this.trackEndTransition = false; this.waitingForQueue = false; this.queueStartPromise = null;
	}
	...
	private async play(query: string | Track | SearchResult | null, requestedBy: string | undefined, rpcContext: PlayerBusRpcContext): Promise<boolean> {
		...
		try {
			...
			if ((this.session?.status === "playing" || this.session?.status === "paused") && !this.waitingForQueue) {
				void this.bus.requestRpc("preload.next", {}, { signal: rpcContext.signal }).catch((error) => player.debug("[Player] Preload after queue add error:", error));
				return true;
			}
	...
	private async prepareRelated(session: PlaybackSession, context: PlayerMessageContext): Promise<void> {
		...
		} catch (error) { if (!context.signal.aborted && this.matchesContext(session, context)) this.bus.event({ type: "TRACK_ERROR", session: session.snapshot(), error: error instanceof Error ? error : new Error(String(error)) }); }
	}
		...
		try {
			...
			if (this.bus.querySync("queueAutoPlay")) {
				...
				if (candidate && this.session?.id === snapshot.id && this.session.isActive()) {
					endedSession.markEnded(); this.bus.requestRpcSync("queue.willNext", { track: null });
	...
	private async skip(context: PlayerMessageContext) {
		if (context.signal.aborted) return;
		const from = this.session?.track ?? null; const oldSession = this.session;
		...
		try {
			...
			if (!next && this.bus.querySync("queueAutoPlay") && oldSession) {
				...
				if (candidate) { this.bus.requestRpcSync("queue.willNext", { track: null }); if (!this.bus.querySync("queueNextTrack")) this.bus.requestRpcSync("queue.addMultiple", { tracks: [candidate] }); next = await this.nextThroughBus(true, context); }
			}
	...
	private async start(track: Track, parentContext: PlayerMessageContext, from: Track | null = null) {
		...
		this.session = new PlaybackSession(); const x = this.session; x.begin(track);
		...
		try {
			...
			if (filterString && this.o.filterController) { const filtered = await this.filterStreamThroughBus(loaded.stream, 0, context); if (!filtered?.stream) throw new Error("Filter controller produced no stream"); activeStream = filtered; }
			let streamToPlay: import("stream").Readable; let inputType = activeStream.inputType;
			if (this.o.streamController && (activeStream.stream || activeStream.url || activeStream.recreate)) { const active = await this.o.streamController.replace(activeStream, x); if (context.signal.aborted || !this.matchesContext(x, context)) return; streamToPlay = active.stream; if (active.inputType) inputType = active.inputType; } else streamToPlay = activeStream.stream as import("stream").Readable;
			const resource = this.bus.requestRpcSync<{ stream: import("stream").Readable; track: Track; inputType?: import("@discordjs/voice").StreamType }, import("@discordjs/voice").AudioResource>("resource.create", { stream: streamToPlay, track, inputType });
