...
P.applyTrackMiddleware = async function (this: Player, track: Track) { const m: any = (this as any).options?.trackMiddleware; if (Array.isArray(m)) for (const fn of m) { const r = await fn(track, { player: this, manager: this.manager }); if (r && r !== track) Object.assign(track, r); } return track; };
...
P.formatTime = function (this: Player, ms: number): string { const totalSeconds = Math.floor(toMilliseconds(ms) / 1000); const hours = Math.floor(totalSeconds / 3600); const minutes = Math.floor((totalSeconds % 3600) / 60); const seconds = totalSeconds % 60; if (hours > 0) return `${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`; return `${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`; };
...
P.getProgressBar = function (this: Player, options: any = {}): string { const { size = 20, barChar = "▬", progressChar = "🔘", timeFormat = "compact", showPercentage = false, showTime = true, hideProgressChar = false } = options; const length = Math.max(1, Math.floor(Number(size) || 20)); const track = this.currentTrack; if (track?.isLive) return "🔴 LIVE"; if (!track) return ""; const total = toMilliseconds(track.duration); if (total <= 0) return showTime ? `${timeFormat === "full" ? "00:00" : "0:00"} ${barChar.repeat(length)} ${timeFormat === "full" ? "00:00" : "0:00"}` : barChar.repeat(length); const resource = this.currentResource ?? this.playbackController?.activeResource; const current = Math.min(total, toMilliseconds(resource?.playbackDuration)); const ratio = current / total; const cursorIndex = Math.min(length - 1, Math.floor(ratio * (length - 1))); const bar = hideProgressChar || progressChar === "none" ? barChar.repeat(length) : barChar.repeat(cursorIndex) + progressChar + barChar.repeat(length - cursorIndex - 1); const format = timeFormat === "full" ? this.formatTime : this.formatTimeCompact; let result = showTime ? `${format(current)} ${bar} ${format(total)}` : bar; if (showPercentage) result += ` (${Math.round(ratio * 100)}%)`; return result; };
