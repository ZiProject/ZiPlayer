...
/**
 * Compatibility runtime used while legacy Player.ts is being reduced to a facade.
 * It centralizes the four migrated policies without duplicating stream/audio ownership.
 */
		...
		try {
			return await this.host.antiStuckController.recoverTrack(track, signal, reason, {
				retry: async ({ retry, signal: retrySignal }) => {
					...
					if (this.host.antiStuckController.policy.reduceQualityOnRetry) {
						this.host.options.quality = "low";
					}
					try {
	...
	public async playNext(skipLoop = false): Promise<boolean> {
		...
		while (!this.host.destroyed) {
			...
			try {
				...
				if (this.host.antiStuckController.policy.enabled && failures < this.host.antiStuckController.policy.controlledSkipThreshold) {
