...
export interface PlayerRuntimeGraph { connectionController: ConnectionController; lifecycleController: LifecycleController; forwardController: ForwardController; queue: Queue; audioPlayer: AudioPlayer; streamManager: StreamManager; preloadManager: PreloadManager; pluginManager: PluginManager; extensionManager: ExtensionManager; queueController: QueueController; trackLoader: TrackLoader; playbackController: PlaybackController; streamController: StreamController; saveController: SaveController; filterController: FilterController; antiStuckController: AntiStuckController; transitionController: TransitionController; volumeController: VolumeController; preloadController: PreloadController; orchestrator: PlaybackOrchestrator; ttsController: TTSController; debugTracer: PlayerEventDebug; searchController: SearchController; eventBridge: PlayerEventBridge; }

/** Composition root and lifecycle owner. It contains no playback workflow. */
export class PlayerRuntimeController {
	private disposed = false; private readonly disposables = new Map<string, () => void | Promise<void>>(); private readonly errors: Array<{ name: string; error: unknown }> = [];
	...
	public initialize(player: Player, manager: PlayerManager, options: PlayerOptions, debug: (...args: any[]) => void): PlayerRuntimeGraph {
		...
		const preloadManager = new PreloadManager({ streamManager, debug, getNextTrack: () => queue.loop() === "track" ? queue.currentTrack : queue.nextTrack, getStream: (track) => resolver.resolve(player, track), removeTrackFromQueue: (track) => { const next = queue.nextTrack; return (next === track || (next?.id !== undefined && track.id !== undefined && next.id === track.id) || (next?.url !== undefined && track.url !== undefined && next.url === track.url)) ? queue.remove(0) !== null : false; }, isDestroyed: () => player.destroyed, isEnabled: () => options.preload?.enabled ?? true });
	...
	public async dispose(): Promise<void> { if (this.disposed) return; this.disposed = true; this.errors.length = 0; for (const [name, cleanup] of [...this.disposables.entries()].reverse()) { try { await cleanup(); } catch (error) { this.errors.push({ name, error }); } } this.disposables.clear(); }
