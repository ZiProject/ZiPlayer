...
export class StreamController {
	...
	public constructor(options: StreamControllerOptions = {}) { this.streamManager = options.streamManager; this.bus = options.bus; }
	public get current(): ActiveStream | null { return this.active; }
	public async resolve(info: StreamInfo, session: PlaybackSession): Promise<Readable> {
		...
		if (info.recreate) {
			...
			if (!session.isActive()) { stream.destroy(); throw this.abortError(); }
			return stream;
		}
	...
	public async replace(info: StreamInfo, session: PlaybackSession): Promise<ActiveStream> {
		...
		const streamId = this.streamManager?.registerStream(stream, session.track!, {
			source: session.track?.source, isPreload: false, isRemote: info.remote ?? false, priority: 10,
		});
	...
	public abortCurrent(): void { if (this.active) this.abort(this.active); }
	public abort(stream: ActiveStream): void {
		...
		if (stream.streamId) { this.streamManager?.unregisterStream(stream.streamId, true); return; }
		if (!stream.stream.destroyed) { try { stream.stream.destroy(); } catch {} }
	}
	public dispose(): void { this.abortCurrent(); this.active = null; }
	private abortError(): Error { const error = new Error("Playback stream operation was aborted"); error.name = "AbortError"; return error; }
}
