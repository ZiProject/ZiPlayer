...
export class PlayerRuntimeController {
 public readonly bus=new PlayerBus(); public readonly actionExecutor=new PlayerAction(this.bus); public readonly connectionController:ConnectionController; public readonly lifecycleController:LifecycleController; public readonly forwardController:ForwardController; public readonly queue:Queue; public readonly audioPlayer:AudioPlayer; public readonly streamManager:StreamManager; public readonly preloadManager:PreloadManager; public readonly queueController:QueueController; public readonly trackLoader:TrackLoader; public readonly playbackController:PlaybackController; public readonly streamController:StreamController; public readonly filterController:FilterController; public readonly antiStuckController:AntiStuckController; public readonly transitionController:TransitionController; public readonly volumeController:VolumeController; public readonly preloadController:PreloadController; public readonly orchestrator:PlaybackOrchestrator; private disposed=false; private readonly player:Player; private readonly detachResourceRefresh:()=>void;
 public constructor(o:PlayerRuntimeControllerOptions){
  const {player,manager,options}=o; this.player=player;
  ...
  this.preloadManager=new PreloadManager({streamManager:this.streamManager,debug:o.debug,getNextTrack:()=>this.queue.loop()==="track"?this.queue.currentTrack:this.queue.nextTrack,getStream:t=>resolver.resolve(player,t),removeTrackFromQueue:t=>{const n=this.queue.nextTrack;return n===t||(n?.id!==undefined&&t.id!==undefined&&n.id===t.id)||(n?.url!==undefined&&t.url!==undefined&&n.url===t.url)?this.queue.remove(0)!==null:false},isDestroyed:()=>this.disposed,isEnabled:()=>options.preload?.enabled??true});
  this.trackLoader=new TrackLoader({middleware,context:{player,manager},resolvers:[t=>resolver.resolve(player,t)],recovery:options.antiStuck,preloadManager:this.preloadManager,debug:o.debug});
  ...
  this.volumeController=new VolumeController(this.bus,{initialVolume:(options as any).volume??100,loudness:(options as any).loudness});
  ...
  this.filterController=new FilterController({refreshPlayerResource:(position)=>this.bus.request({type:"[Player]->[Resource]:refresh",requestId:createPlayerRequestId(),position},{timeoutMs:30000}).then(()=>true).catch(()=>false)},o.debug);
  this.detachResourceRefresh=this.bus.onInput("[Player]->[Resource]:refresh",(event)=>{void this.handleResourceRefresh(event);});
  this.bus.subscribe("volumeRequested",(value:number)=>this.volumeController.setVolume(value));
 ...
 private async handleResourceRefresh(event:Extract<PlayerInput,{type:"[Player]->[Resource]:refresh"}>):Promise<void>{
  try{
   ...
   this.filterController.setSourceStreamType(info.type??"arbitrary");
   const processed=await this.filterController.applyFiltersAndSeek(info,Math.max(0,position));
   ...
   session.setResource(resource); this.playbackController.play(resource,session); session.markPlaying(Math.max(0,position));
 ...
 public dispose():void{if(this.disposed)return;this.disposed=true;this.detachResourceRefresh();this.filterController.destroy();this.lifecycleController.dispose();this.forwardController.dispose();this.volumeController.dispose();void this.connectionController.dispose();this.actionExecutor.dispose();this.orchestrator.dispose();this.preloadController.dispose();this.preloadManager.cancelPreload();this.streamController.dispose();this.streamManager.destroyAll();this.queueController.dispose();this.playbackController.dispose();this.bus.publish("destroyed");this.bus.clear();}
}