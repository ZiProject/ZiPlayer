...
async function generatePoToken(contentBinding: string, signal?: AbortSignal): Promise<string> {
	...
	const botGuardClient = await BotGuardClient.create({ program: challenge.program, globalName: challenge.globalName, globalObject: globalThis });
	const webPoSignalOutput: WebPoSignalOutput = [];
	const botguardResponse = await botGuardClient.snapshot({ webPoSignalOutput });
	const response = await fetch(buildURL("GenerateIT", true), {
		method: "POST", headers: getHeaders(), body: JSON.stringify([requestKey, botguardResponse]), signal,
	});
	if (!response.ok) throw new Error(`GenerateIT failed: HTTP ${response.status}`);
	const [integrityToken, estimatedTtlSecs, mintRefreshThreshold, websafeFallbackToken] = (await response.json()) as [string, number, number, string];
...
async function makePlayerRequest(innertube: Innertube, videoId: string, _reloadPlaybackContext?: ReloadPlaybackContext): Promise<any> {
	return innertube.getBasicInfo(videoId, { client: "YTMUSIC" });
}

function getClientInfo(innertube: Innertube) {
	const clientName = Constants.CLIENT_NAME_IDS[innertube.session.context.client.clientName as keyof typeof Constants.CLIENT_NAME_IDS];
...
async function safeCancelStream(stream: ReadableStream<Uint8Array> | undefined, reason?: unknown): Promise<void> {
	if (!stream) return;
	try { await stream.cancel(reason); } catch {}
}
...
export async function createSabrStream(videoId: string, innertube: Innertube, options?: Partial<SabrPlaybackOptions>, signal?: AbortSignal): Promise<SabrAudioResult> {
	if (signal?.aborted) throw signal.reason ?? new DOMException("The operation was aborted", "AbortError");
	const throwIfAborted = () => { if (signal?.aborted) throw signal.reason ?? new DOMException("The operation was aborted", "AbortError"); };
	...
	try {
		...
		throwIfAborted();
		if (videoInfo.playability_status?.status && videoInfo.playability_status.status !== "OK") throw new Error(`Video is not playable: ${videoInfo.playability_status.status} (${videoInfo.playability_status.reason ?? "no reason given"})`);
		...
		if (!streamingUrl) throw new Error("This video has no SABR streaming URL. YouTube may have returned a legacy/non-SABR response.");
		...
		sabr = new SabrStream({ formats, serverAbrStreamingUrl, videoPlaybackUstreamerConfig: ustreamerConfig, poToken, clientInfo: getClientInfo(innertube) });
		reloadHandler = async (reloadPlaybackContext: ReloadPlaybackContext) => {
			try {
				throwIfAborted();
				const [reloaded, refreshedPoToken] = await Promise.all([makePlayerRequest(innertube, videoId, reloadPlaybackContext), generatePoToken(videoId, signal)]);
				throwIfAborted();
				const newUrl = reloaded.streaming_data?.server_abr_streaming_url ? await innertube.session.player?.decipher(reloaded.streaming_data.server_abr_streaming_url) : undefined;
				...
				const newFormats: SabrFormat[] = (reloaded.streaming_data?.adaptive_formats ?? []).map((format: any) => buildSabrFormat(format));
		...
		const result = await sabr.start({ ...options, enabledTrackTypes: options?.enabledTrackTypes ?? EnabledTrackTypes.AUDIO_ONLY, audioQuality: options?.audioQuality ?? "medium", preferWebM: options?.preferWebM ?? true, preferOpus: options?.preferOpus ?? true });
		throwIfAborted();
		...
		const nodeStream = Readable.from((async function* () {
			try { while (true) { throwIfAborted(); const { done, value } = await reader.read(); if (done) break; yield Buffer.from(value); } }
			finally { try { await reader.cancel(); } catch {} try { reader.releaseLock(); } catch {} safeAbortSabr(sabrSession); void safeCancelStream(result.videoStream, "Video track is not used"); if (reloadHandler) sabrSession.off("reloadPlayerResponse", reloadHandler); }
		})());
		return { title, stream: nodeStream, format: { mimeType: audioFormat.mimeType ?? "audio/webm", itag: Number(audioFormat.itag ?? 0), contentLength: Number(audioFormat.contentLength ?? 0) } };
	} catch (error) {
		if (sabr) { if (reloadHandler) sabr.off("reloadPlayerResponse", reloadHandler); safeAbortSabr(sabr); }
		throw new Error(`SABR stream creation failed: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
	}
}

/**
 * Creates a raw video Readable from the same SABR session used by audio.
 * SABR is requested in VIDEO_ONLY mode, so YouTube sends video segments directly
 * and we do not have to fetch/mux a separate direct URL.
 */
export async function createSabrVideoStream(videoId: string, innertube: Innertube, options?: Partial<SabrPlaybackOptions>, signal?: AbortSignal): Promise<SabrVideoResult> {
	if (signal?.aborted) throw signal.reason ?? new DOMException("The operation was aborted", "AbortError");
	const throwIfAborted = () => { if (signal?.aborted) throw signal.reason ?? new DOMException("The operation was aborted", "AbortError"); };
	let sabr: SabrStream | undefined;
	...
	try {
		...
		const videoInfo = await makePlayerRequest(innertube, videoId);
		throwIfAborted();
		if (videoInfo.playability_status?.status && videoInfo.playability_status.status !== "OK") throw new Error(`Video is not playable: ${videoInfo.playability_status.status}`);
		const title = videoInfo.video_details?.title ?? videoInfo.basic_info?.title ?? videoId;
		const streamingUrl = videoInfo.streaming_data?.server_abr_streaming_url;
		...
		reloadHandler = async (ctx: ReloadPlaybackContext) => {
			try {
				throwIfAborted();
				const [reloaded, token] = await Promise.all([makePlayerRequest(innertube, videoId, ctx), generatePoToken(videoId, signal)]);
				const newUrl = reloaded.streaming_data?.server_abr_streaming_url ? await innertube.session.player?.decipher(reloaded.streaming_data.server_abr_streaming_url) : undefined;
				const newConfig = reloaded.player_config?.media_common_config?.media_ustreamer_request_config?.video_playback_ustreamer_config;
				const newFormats: SabrFormat[] = (reloaded.streaming_data?.adaptive_formats ?? []).map((format: any) => buildSabrFormat(format));
				...
				if (newConfig) sabr?.setUstreamerConfig(newConfig);
				if (newFormats.length) sabr?.setServerAbrFormats(newFormats);
			} catch (error) { if (!signal?.aborted) console.error("[SABR] Video reload failed:", error); }
		};
		sabr.on("reloadPlayerResponse", reloadHandler);
		...
		const nodeStream = Readable.from((async function* () {
			try { while (true) { throwIfAborted(); const { done, value } = await reader.read(); if (done) break; yield Buffer.from(value); } }
			finally { try { await reader.cancel(); } catch {} try { reader.releaseLock(); } catch {} safeAbortSabr(sabrSession); if (reloadHandler) sabrSession.off("reloadPlayerResponse", reloadHandler); }
		})());
		return { title, stream: nodeStream, format: { mimeType: videoFormat.mimeType ?? "video/mp4", itag: Number(videoFormat.itag ?? 0), contentLength: Number(videoFormat.contentLength ?? 0) } };
	} catch (error) {
		if (sabr) { if (reloadHandler) sabr.off("reloadPlayerResponse", reloadHandler); safeAbortSabr(sabr); }
		throw new Error(`SABR video stream creation failed: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
	}
}
...
function sanitizeFileName(name: string): string { return name.replace(/[<>:"/\\|?*\x00-\x1F]/g, "_").trim(); }
function getExtensionFromMimeType(mimeType: string): string {
	const normalized = mimeType.toLowerCase().split(";")[0];
	return ({ "audio/mpeg": "mp3", "audio/mp4": "m4a", "audio/webm": "webm", "audio/ogg": "ogg", "video/mp4": "mp4", "video/webm": "webm" } as Record<string, string>)[normalized] ?? "bin";
}
