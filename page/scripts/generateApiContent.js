...
function kindOf(reflection) {
	if (reflection.kindString) return reflection.kindString; // phòng khi build với typedoc cũ hơn
	return KIND_NAME_BY_VALUE[reflection.kind] || String(reflection.kind) || "symbol";
}
...
function eventsOf(reflection) {
	...
	return (reflection.children || [])
		...
		.map((event) => {
			...
			const parameters = type?.type === "tuple" ? (type.elements || []).map((element) => typeString(element) || "unknown") : [];
...
function publicFromOf(reflection, exportGraph) {
	...
	for (const [file, names] of exportGraph) {
		...
		const fileMatch = [...candidates].some(
			(candidate) =>
				normalizedFile === candidate || normalizedFile.endsWith(`/${candidate}`) || candidate.endsWith(`/${normalizedFile}`),
		);
